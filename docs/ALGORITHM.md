...
The agent MUST report with a caveat and defer for confirmation. Use **bold** for the version and `[PR #N](url)` for the link. The agent MUST include a `Confidence: **Medium**` line followed by the reason on the same line after the main finding.
